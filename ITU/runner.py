...
import pyspiel
from state import InfoState, card_num_to_str
...
def play_tournament_table(bots, start_stack: int, use_timeout=True, console_output=False, calc_win_chance=False):
    ...
    while len(active_players) > 1:
        ...
        rewards, json_hand_events = play_hand(active_players, get_blinds_input(current_blinds, len(active_players)),
                                              use_timeout, console_output, calc_win_chance)
        json_hand["hand_events"] = json_hand_events

        if hand_count == current_blinds.next_blind_change:
            current_blinds = next(blinds_iter)

        newly_defeated_players, active_players = update_active_players(active_players, rewards,
                                                                       current_blinds.big_blind)

        defeated_players = defeated_players + newly_defeated_players
        if len(active_players) < 1:
            print(newly_defeated_players)
            print(len(newly_defeated_players))
...
def update_active_players(active_players: List[Player], rewards: List[int], big_blind: int):
    updated_players = [Player(player.bot_impl, int(player.stack + r), player.id) for player, r in
                       zip(active_players, rewards)]
...
def play_hand(players: List[Player], blinds: List[int], use_timeut, console_output=False, calc_win_chance=False):
    ...
    while not state.is_terminal():
        ...
        apply_player_action(state, info_state, json_events, current_idx, action, players)
        if console_output:
        ...
        for i, reward in enumerate(state.rewards()):
            ...
            if info_state.player_infos[i].active:
                ...
                if len(info_state.board_cards) == 5:
                    print_str += " with a " + get_hand_type(
                        list(info_state.board_cards) + list(info_state.player_hands[i])).name
                print(print_str)
            else:
...
def apply_player_action(state, info_state, json_events, current_idx, action, players):
...
def init_game(players: List[Player], blinds, console_output):
    ...
    if console_output:
        ...
        [print(players[int(i / 2)].bot_impl.get_name() + " hand: " + cards[i] + cards[i + 1]) for i in
         range(0, len(cards), 2)]
